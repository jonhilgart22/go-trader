...
class BollingerBandsSimulator:
    ...
    def _add_additional_training_dfs(self, ts_stacked_series, additional_dfs, verbose=False):
        ...
        for df in additional_dfs:
            ...
            if all_ts_stacked_series is None:
                if verbose:
                    print('last date for training additional df data',
    ...
    def _check_ml_prediction(self, end_time,   tcn_first_epochs=200, tcn_sub_epochs=100, nbeats_first_epochs=10,
            ...
            for model_n in self.model_name:
                for lookback_window in self.ml_lookback_windows:
                    if model_n == "NBEATS":
                        ...
                        raise ValueError
                        print(
                            f"Error. Incorrect input model of {self.model_name}")
        ...
        ts_transformers, ts_stacked_series, train_close_series = self._scale_time_series_df_and_time_cols(
            training_df)
        if verbose:
            print('original DF training series',
                  ts_stacked_series.components)
        ...
        if len(self.additional_dfs) > 0:
            ...
            ts_transformers = {**additional_ts_transformers,
                               **ts_transformers}  # merge dicts
    ...
    def _check_if_we_should_buy(self, index, row):
        ...
        if ml_pred > row['Rolling Mean']:
            ...
            self.df['ML_Future_Prediction'] = ml_pred
            self.ml_prediction_date_and_price[row.name +
                                              timedelta(days=self.ml_prediction_n_days)] = ml_pred
    ...
    def _calculate_returns(self):
        ...
        self.df['Strategy Return'] = self.df['Market Return'] * \
            self.df['Position']
    ...
    def _slice_df(self):
        ...
        self.prediction_start = pd.to_datetime(
            self.from_date) + timedelta(self.max_looback)

        sliced_additional_dfs = []
        if len(self.additional_dfs) > 0:
            for add_df in self.additional_dfs:
                sliced_df = add_df.loc[pd.to_datetime(
                    self.from_date):, :].copy()
